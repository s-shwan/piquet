...
class PiquetGame {
    ...
    doEcarte() {
        ...
        if (selectedCards.length === 0 && this.declarer === 0) {
            alert('Select cards to discard, or click again to pass.');
            return;
        }
    ...
    autoEcarte() {
        if (this.declarer === 1 && this.phase === 'ecarte') {
            // Player 2 AI discards low cards up to the number P1 discarded, capped by talon size
            let p2 = this.players[1];
            let cardsToDiscard = [];

            if (this.talon.length > 0) {
                 // Simple AI: Discard lowest cards
                cardsToDiscard = p2.hand.slice(p2.hand.length - 2); 
            } 
            
            // Cap the discard count by remaining talon cards
    ...
    endRound() {
        ...
        if (this.tricksWon[0] === 12) {
            ...
            if (this.tricksWon[0] > this.tricksWon[1]) {
                 this.players[0].score += 10; // 10 points for most tricks
    ...
    updateUI() {
        ...
        // Determine whose hand should be displayed and who is acting
        const isP1Acting = this.phase === 'ecarte' || (this.phase === 'tricks' && this.leader === 0) || (this.phase === 'tricks' && this.leader === 1 && this.trickCards.length === 1);

        document.getElementById('current-player').innerText = isP1Acting ? `Player 1's turn (${this.phase})` : `Player 2 (AI) is playing...`;
        
        let handDiv = document.getElementById('hand-display');
        handDiv.innerHTML = '';
        
        // Controls visibility
        document.getElementById('ecarte-btn').style.display = (this.phase === 'ecarte' && this.declarer === 0) ? 'block' : 'none';
        document.getElementById('declare-btn').style.display = (this.phase === 'declare') ? 'block' : 'none';
        
        // Only display cards for the human player (Player 1)
        if (isP1Acting && this.declarer === 0) {
